...
def run_mpc(task_id=0, seed=0, wm_client=None):
    """Run one episode of VLM-guided MPC with trajectory optimisation.

    Phases:
        1. Execute diffusion policy for subtask 0 until the handoff condition.
        2. Query VLM agent for a goal point and optimise the reaching trajectory
           (rotation → midpoint → endpoint).
        3. Resume diffusion policy for subtask 1 to completion.

    Returns:
        bool: True if the task was completed successfully.
    """
    save_dir = os.path.join("scratch_dir/mpc_data/test_agent_optimization", f"task{task_id}", f"seed{seed}")
    ...
    with open(log_file_path, 'w') as log_file:
        with contextlib.redirect_stdout(log_file):
            ...
            # -- Step 1: Optimise gripper rotation --
            ...
            subtask_id = 1
            while not done and num_steps < 650:
                action_chunk = policy_fn(obs, subtask_id=subtask_id)[:10]
                for action in action_chunk:
...
if __name__ == "__main__":
    ...
    for task in [7]:
        ...
        with open(os.path.join("scratch_dir/mpc_data/test_agent_optimization", f"task{task}", "result.txt"), "w") as f:
